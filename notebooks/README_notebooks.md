...
This directory contains curated Jupyter notebooks demonstrating **VQE**, **ADAPT-VQE**, **LR-VQE**, **EOM-VQE**, **QSE**, **EOM-QSE**, **VQD/SSVQE** (excited states), **QPE**, and **VarQITE** workflows using the packaged code in:
...
A small number of notebooks additionally construct **exact reference spectra** locally (e.g., via `pennylane.qchem`) for validation plots and ΔE sanity checks.
...
| `EOM_QSE.ipynb`                     | **Equation-of-Motion QSE (EOM-QSE)**: commutator EOM in an operator manifold vs exact eigenvalues (noiseless)   | Package client                       |
| `LR_VQE.ipynb`                      | **Linear-Response VQE (LR-VQE)**: tangent-space TDA excited energies vs exact eigenvalues (noiseless)           | Package client                       |
| `EOM_VQE.ipynb`                     | **Equation-of-Motion VQE (EOM-VQE)**: tangent-space full-response excited energies vs exact eigenvalues (noiseless) | Package client                    |
...
- `LR_VQE.ipynb` demonstrates **LR-VQE** as a *post-processing* method built on a converged VQE reference state, by solving a tangent-space generalized eigenvalue problem (TDA).
- `EOM_QSE.ipynb` demonstrates **EOM-QSE** as a *post-processing* method built on a converged VQE reference state, by solving the commutator EOM generalized EVP in an operator manifold (generally non-Hermitian; selects physical roots).
- `EOM_VQE.ipynb` demonstrates **EOM-VQE** as a *post-processing* method built on a converged VQE reference state, by solving the full-response tangent-space EOM problem (positive-root selection).
...
4. **Excited states**
   - `vqe/H2/QSE.ipynb` (post-VQE operator subspace expansion)
   - `vqe/H2/EOM_QSE.ipynb` (post-VQE commutator EOM in operator manifold)
   - `vqe/H2/LR_VQE.ipynb` (post-VQE tangent-space linear response; TDA)
   - `vqe/H2/EOM_VQE.ipynb` (post-VQE tangent-space full response)
